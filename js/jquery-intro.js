...
$(document).ready(function () {
    ...
    // For testing, you can use one of these instead:
    // var today = new Date("December 15, 2026");
    // var today = new Date("July 15, 2026");

    // Get month number and month name
    var monthNum = today.getMonth();
    var monthName = months[monthNum];

    // If December, add Happy Holidays after the slogan h2
    if (monthNum === 11) {
        $("#slogan").after("<h3>Happy Holidays!</h3>");
    }

    // Change the month heading
    $("#month").html("Tips for " + monthName);

    // Add current year to copyright paragraph
    var year = today.getFullYear();
    $("#copy").append(" " + year);

    // Display monthly specials
    ...
    // Change seasonal tips
    $("#seasontips").html(tips[seasonIndex]);

    // Change colors of strong, h1, h2, h3
